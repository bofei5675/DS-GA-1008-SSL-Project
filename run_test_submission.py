...
from helper_submission import compute_ats_bounding_boxes, compute_ts_road_map

from model_loader import get_transform_task1, get_transform_task2, ModelLoaderSubmission
...
if opt.det_model is None or opt.seg_model is None:
    model_loader = ModelLoaderSubmission()
else:
    model_loader = ModelLoaderSubmission(opt.det_model, opt.seg_model)
...
with torch.no_grad():
    for i, data in enumerate(dataloader_task1):
        ...
        if opt.verbose:
            print(f'{i} - Bounding Box Score: {ats_bounding_boxes:.4} / {total_ats_bounding_boxes / total}')
    total_road_num = 0
    for i, data in enumerate(dataloader_task2):
        ...
        if opt.verbose:
            print(f'{i} - Road Map Score: {ts_road_map:.4} / {total_ts_road_map / total_road_num}')
...
with open(model_loader.debug_det +'/eval_{}.txt'.format(model_loader.threshold), 'w') as f:
